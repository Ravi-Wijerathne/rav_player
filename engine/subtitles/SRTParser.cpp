...
bool SRTParser::load(const std::string& content) {
    raw_content_ = content;
    return true;
}

bool SRTParser::load_file(const std::string& path) {
...
std::vector<SubtitleFrame> SRTParser::parse() {
    frames_.clear();
...
double SRTParser::time_to_seconds(int h, int m, int s, int ms) {
