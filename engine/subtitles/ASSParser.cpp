...
bool ASSParser::load(const std::string& content) {
    raw_content_ = content;
    return true;
}

bool ASSParser::load_file(const std::string& path) {
...
std::vector<SubtitleFrame> ASSParser::parse() {
    frames_.clear();
...
std::string ASSParser::strip_ass_overrides(const std::string& text) {
    std::string result;
